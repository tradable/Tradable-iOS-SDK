...
SWIFT_MODULE_NAMESPACE_PUSH("TradableAPI")
...
@class TradableStartTrialRequest;
@class TradableSubscriptionResponse;
@class TradableSubscriptionRequest;
@class TradableCancelSubscriptionRequest;
@class TradableTwoFactorAuthenticationChallenge;
@class TradableTwoFactorAuthenticationResponse;
@class TradableApp;
...
@property (nonatomic, weak) id <TradableEventsDelegate> _Nullable eventsDelegate;
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) Tradable * _Nonnull sharedInstance;)
+ (Tradable * _Nonnull)sharedInstance SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
/// A method used to create and/or activate DEMO accounts.
/// Tries to activate Tradable SDK with the last known tokens; if fails, falls back to creating a demo account.
...
/// \param type The account type to be used or created (stocks or forex).
///
/// \param appId The ID of the app.
///
/// \param appKey The key of the app.
///
- (void)activateOrCreateDemoAccountOfType:(enum TradableDemoAccountType)type forAppId:(uint64_t)appId withAppKey:(NSString * _Nonnull)appKey;
/// A method used to create and/or activate LIVE accounts.
/// Tries to activate Tradable SDK with the last known tokens; if fails, falls back to authentication flow.
...
/// \param viewController Optional UIViewController conforming to SFSafariViewControllerDelegate component used to transition to SFSafariViewController with login website. If not specified, system browser will be opened.
///
- (void)authenticateWithAppId:(uint64_t)appId uri:(NSString * _Nonnull)uri viewController:(UIViewController * _Nullable)viewController showLogin:(BOOL)showLogin;
...
///
- (void)activateWith:(TradableAccessToken * _Nonnull)accessToken;
...
///
/// \param completion The closure to be called when the response comes back, with an optional TradableAccessToken object and an optional TradableError object.
///
- (void)authenticateWith:(TradableAPIAuthenticationRequest * _Nonnull)authRequest completion:(void (^ _Null_unspecified)(TradableAccessToken * _Nullable, TradableError * _Nullable))completion;
...
/// \param completion The closure to be called when the response comes back, with an optional TradableAccessToken object and an optional TradableError object.
///
- (void)createDemoAccountWith:(TradableDemoAPIAuthenticationRequest * _Nonnull)demoAuthRequest completion:(void (^ _Null_unspecified)(TradableAccessToken * _Nullable, TradableError * _Nullable))completion;
...
/// \param completion The closure to be called when the response comes back, with an optional TradableAccessToken object and an optional TradableError object.
///
- (void)refreshAuthenticationWith:(TradableAPIRefreshAuthenticationRequest * _Nonnull)refreshAuthRequest completion:(void (^ _Null_unspecified)(TradableAccessToken * _Nullable, TradableError * _Nullable))completion;
/// Returns a list of active accounts.
- (NSArray<TradableAccount *> * _Nonnull)getActiveAccounts SWIFT_WARN_UNUSED_RESULT;
...
- (NSDictionary<NSString *, TradableAccessToken *> * _Nullable)getLastSessionAccessTokens SWIFT_WARN_UNUSED_RESULT;
/// Requests starting a trial period in order to get access to trading with the given broker.
/// \param trialRequest The request for starting the subscription.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableSubscriptionResponse object and an optional TradableError object.
///
- (void)startTrialWith:(TradableStartTrialRequest * _Nonnull)trialRequest completion:(void (^ _Null_unspecified)(TradableSubscriptionResponse * _Nullable, TradableError * _Nullable))completion;
/// Subscribes in order to get access to trading with the given broker.
/// \param subscriptionRequest The request for starting the subscription.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableSubscriptionResponse object and an optional TradableError object.
///
- (void)subscribeWith:(TradableSubscriptionRequest * _Nonnull)subscriptionRequest completion:(void (^ _Null_unspecified)(TradableSubscriptionResponse * _Nullable, TradableError * _Nullable))completion;
/// Retrieves information on a subscription.
/// \param brokerId The id of the broker that the account is at.
///
/// \param userLogin The login for the account.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableSubscriptionResponse object and an optional TradableError object.
///
- (void)getSubscriptionInfoForBrokerId:(NSInteger)brokerId andUserLogin:(NSString * _Nonnull)userLogin completion:(void (^ _Null_unspecified)(TradableSubscriptionResponse * _Nullable, TradableError * _Nullable))completion;
/// Cancels the subscription for the given user.
/// \param cancelSubscriptionRequest The request for cancelling the subscription.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableSubscriptionResponse object and an optional TradableError object.
///
- (void)cancelSubscriptionWith:(TradableCancelSubscriptionRequest * _Nonnull)cancelSubscriptionRequest completion:(void (^ _Null_unspecified)(TradableSubscriptionResponse * _Nullable, TradableError * _Nullable))completion;
/// Used to complete two factor authentication.
/// \param account The account for which the two factor authentication response is required.
///
/// \param challenge The two factor authentication challenge that has been received.
///
/// \param twoFactorAuthResponse The response for the challenge.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableAccessToken object and an optional TradableError object.
///
- (void)continueTwoFactorAuthFor:(TradableAccount * _Nonnull)account for:(TradableTwoFactorAuthenticationChallenge * _Nonnull)challenge with:(TradableTwoFactorAuthenticationResponse * _Nonnull)twoFactorAuthResponse completion:(void (^ _Null_unspecified)(TradableAccessToken * _Nullable, TradableError * _Nullable))completion;
/// Gets the app information.
/// \param appId The ID of the app to retrieve information for.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableApp object and an optional TradableError object.
///
- (void)getAppInfoForAppId:(uint64_t)appId completion:(void (^ _Null_unspecified)(TradableApp * _Nullable, TradableError * _Nullable))completion;
/// Gets the current user.
/// \param accessToken The access token to be used when calling the API.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableOSUser object and an optional TradableError object.
///
- (void)getCurrentUserWith:(TradableAccessToken * _Nonnull)accessToken completion:(void (^ _Null_unspecified)(TradableOSUser * _Nullable, TradableError * _Nullable))completion;
/// Gets available accounts.
/// \param accessToken The access token to be used when calling the API.
///
/// \param completion The closure to be called when the response comes back, with an optional TradableAccountList object and an optional TradableError object.
///
- (void)getAvailableAccountsWith:(TradableAccessToken * _Nonnull)accessToken completion:(void (^ _Null_unspecified)(TradableAccountList * _Nullable, TradableError * _Nullable))completion;
/// Gets the indicators information.
/// \param account The account for which the indicators info should be fetched.
///
/// \param completion The closure to be called when the response comes back, with an optional list of TradableIndicatorInfo objects and an optional TradableError object.
///
- (void)getIndicatorsInfoFor:(TradableAccount * _Nonnull)account completion:(void (^ _Null_unspecified)(NSArray<TradableIndicatorInfo *> * _Nullable, TradableError * _Nullable))completion;
@end
...
@property (nonatomic, readonly, copy) NSDate * _Nullable refreshTokenExpirationDate;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
/// Creates an object with given parameters.
...
- (NSArray<TradableAccount *> * _Nonnull)getAccounts SWIFT_WARN_UNUSED_RESULT;
...
enum TradableUpdatesType : NSInteger;
enum TradableUpdatesFrequency : NSInteger;
@class TradableUpdatesRequest;
...
@property (nonatomic, readonly) BOOL isStopLossSupported;
/// Specifies if positions on this account support take profit protections.
@property (nonatomic, readonly) BOOL isTakeProfitSupported;
...
- (TradableAccessToken * _Nullable)getAccessToken SWIFT_WARN_UNUSED_RESULT;
...
///
/// \param request The request that should be used for updates.
///
- (void)startUpdatesOfType:(enum TradableUpdatesType)type withFrequency:(enum TradableUpdatesFrequency)frequency with:(TradableUpdatesRequest * _Nonnull)request;
...
/// \param updatesRequest The request that should be used to fetch the snapshot.
///
...
///
- (void)getAccountSnapshotWith:(TradableUpdatesRequest * _Nonnull)updatesRequest completionHandler:(void (^ _Null_unspecified)(TradableAccountSnapshot * _Nullable, TradableError * _Nullable))completionHandler;
/// Gets prices for this account.
/// \param updatesRequest The request that should be used to fetch the prices.
///
...
///
- (void)getPricesWith:(TradableUpdatesRequest * _Nonnull)updatesRequest completionHandler:(void (^ _Null_unspecified)(TradablePrices * _Nullable, TradableError * _Nullable))completionHandler;
...
///
- (void)getOrderWithId:(NSString * _Nonnull)id completionHandler:(void (^ _Null_unspecified)(TradableOrder * _Nullable, TradableError * _Nullable))completionHandler;
/// Issues a new order.
...
///
- (void)issueNewOrderWith:(TradableOrderCommand * _Nonnull)orderCommand completionHandler:(void (^ _Null_unspecified)(TradableOrder * _Nullable, TradableError * _Nullable))completionHandler;
...
/// \param completionHandler The closure to be called when the response comes back, with an optional TradableOrder object and an optional TradableError object. TradableOrder is an order that will have been created. Getting it back in the closure doesn’t mean that the order has been executed; check its status in tradableOrdersUpdated delegate method.
///
- (void)issueNewMarketOrderFor:(TradableInstrument * _Nonnull)instrument withAmount:(double)amount withSide:(enum TradableOrderSide)side orderDelegate:(id <TradableOrderStatusDelegate> _Nullable)orderDelegate completionHandler:(void (^ _Null_unspecified)(TradableOrder * _Nullable, TradableError * _Nullable))completionHandler;
...
/// \param completionHandler The closure to be called when the response comes back, with an optional TradableOrder object and an optional TradableError object. TradableOrder is an order that will have been created. Getting it back in the closure doesn’t mean that the order has been executed; check its status in tradableOrdersUpdated delegate method.
///
- (void)issueNewMarketOrderWithProtectionsFor:(TradableInstrument * _Nonnull)instrument withAmount:(double)amount withSide:(enum TradableOrderSide)side withTakeProfit:(TradableOrderProtection * _Nullable)takeProfit withStopLoss:(TradableOrderProtection * _Nullable)stopLoss orderDelegate:(id <TradableOrderStatusDelegate> _Nullable)orderDelegate completionHandler:(void (^ _Null_unspecified)(TradableOrder * _Nullable, TradableError * _Nullable))completionHandler;
...
///
- (void)getPositionWithId:(NSString * _Nonnull)id completionHandler:(void (^ _Null_unspecified)(TradablePosition * _Nullable, TradableError * _Nullable))completionHandler;
/// Gets candles.
/// remark:
/// This method is recursive.
/// \param candleRequest The candle request object containing information about the candles that should be fetched.
///
/// \param completionHandler The closure to be called when the response comes back, with an optional TradableCandleResponse object and an optional TradableError object.
///
- (void)getCandlesWith:(TradableCandleRequest * _Nonnull)candleRequest completionHandler:(void (^ _Null_unspecified)(TradableCandleResponse * _Nullable, TradableError * _Nullable))completionHandler;
...
///
/// \param completionHandler The closure to be called when the response comes back, with an optional dictionary of UInt64 to list of TradableCandle objects and an optional TradableError object.
///
- (void)getGroupedCandlesFor:(TradableInstrument * _Nonnull)instrument aggregation:(enum TradableAggregation)aggregation sessions:(NSUInteger)sessions completionHandler:(void (^ _Null_unspecified)(NSDictionary<NSNumber *, NSArray<TradableCandle *> *> * _Nullable, TradableError * _Nullable))completionHandler;
...
///
- (void)getLastSessionCloseWith:(TradableLastSessionCloseRequest * _Nonnull)lastSessionCloseRequest completionHandler:(void (^ _Null_unspecified)(NSArray<TradableLastSessionClose *> * _Nullable, TradableError * _Nullable))completionHandler;
...
///
- (void)getInstrumentsWith:(TradableInstrumentSearchRequest * _Nonnull)instrumentSearchRequest completionHandler:(void (^ _Null_unspecified)(TradableInstrumentList * _Nullable, TradableError * _Nullable))completionHandler;
/// Creates an object with given parameters. Conforms to NSCoding protocol.
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)decoder;
/// Encoding method, conforming to NSCoding protocol.
- (void)encodeWithCoder:(NSCoder * _Nonnull)aCoder;
...
@property (nonatomic, readonly, strong) TradableAccount * _Nonnull associatedAccount;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
/// Five minutes aggregation.
  TradableAggregationFiveMinutes = 5,
/// Fifteen minutes aggregation.
  TradableAggregationFifteenMinutes = 15,
/// Thirty minutes aggregation.
  TradableAggregationThirtyMinutes = 30,
...
/// Unsupported indicator.
  TradableArgumentErrorUnsupportedIndicator = 3,
/// Incorrect indicator parameter.
  TradableArgumentErrorIncorrectIndicatorParameter = 4,
/// Missing indicator parameter.
  TradableArgumentErrorMissingIndicatorParameter = 5,
};
...
- (void)tradableReadyFor:(TradableAccount * _Nonnull)account;
...
///
- (void)tradableAuthenticationErrorWithError:(TradableError * _Nonnull)error;
/// A delegate hook for two factor authentication challenges handling.
/// \param challenge A TradableTwoFactorAuthenticationChallenge object that contains detailed information about the challenge.
///
- (void)tradableTwoFactorAuthenticationChallengeWithChallenge:(TradableTwoFactorAuthenticationChallenge * _Nonnull)challenge;
@optional
...
///
- (void)getOnLightBrokerLogo:(void (^ _Null_unspecified)(UIImage * _Nullable))logoCompleted;
...
///
- (void)getOnDarkBrokerLogo:(void (^ _Null_unspecified)(UIImage * _Nullable))logoCompleted;
...
- (void)tradableBrokerSignInDismissedWithAccessToken:(TradableAccessToken * _Nonnull)accessToken;
@end


/// A request for cancelling the subscription for the given user.
SWIFT_CLASS("_TtC11TradableAPI33TradableCancelSubscriptionRequest")
@interface TradableCancelSubscriptionRequest : NSObject
/// The id of the broker that the account is at.
@property (nonatomic, readonly) NSInteger brokerId;
/// The login for the account.
@property (nonatomic, readonly, copy) NSString * _Nonnull login;
...
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
/// Creates an object with given parameters.
- (nonnull instancetype)initWithBrokerId:(NSInteger)brokerId login:(NSString * _Nonnull)login password:(NSString * _Nonnull)password OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
/// Creates an object with given parameters.
- (nonnull instancetype)initWithInstrumentId:(NSString * _Nonnull)instrumentId fromDate:(NSDate * _Nonnull)fromDate toDate:(NSDate * _Nonnull)toDate aggregation:(NSInteger)aggregation indicatorRequest:(TradableIndicatorRequest * _Nullable)indicatorRequest OBJC_DESIGNATED_INITIALIZER;
/// Creates an object with given parameters.
...
SWIFT_CLASS("_TtC11TradableAPI22TradableCandleResponse")
@interface TradableCandleResponse : NSObject
/// A list of candles.
...
@property (nonatomic, readonly, strong) TradableAccount * _Nullable associatedAccount;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
/// Error emerged in snapshot updates.
  TradableErrorTypeSnapshot = 0,
/// Error emerged in orders updates.
  TradableErrorTypeOrders = 1,
/// Error emerged in positions updates.
  TradableErrorTypePositions = 2,
/// Error emerged in prices updates.
  TradableErrorTypePrices = 3,
/// Error emerged in candles updates.
  TradableErrorTypeCandles = 4,
...
///
- (void)tradableAccountMetricsUpdatedWithAccountMetrics:(TradableAccountMetrics * _Nonnull)accountMetrics;
/// A delegate hook for orders updates.
/// \param orders A TradableOrders object that contains a list of orders.
///
- (void)tradableOrdersUpdatedWithOrders:(TradableOrders * _Nonnull)orders;
/// A delegate hook for positions updates.
/// \param positions A TradablePositions object that contains a list of positions.
///
- (void)tradablePositionsUpdatedWithPositions:(TradablePositions * _Nonnull)positions;
...
///
- (void)tradablePricesUpdatedWithPrices:(TradablePrices * _Nonnull)prices;
/// A delegate hook for candles updates.
/// \param candles A TradableCandleResponse object that contains a list of candles.
///
- (void)tradableCandlesUpdatedWithCandles:(TradableCandleResponse * _Nonnull)candles;
/// A delegate hook for events error handling.
/// \param error A TradableError object that contains detailed information about the error.
///
- (void)tradableEventsErrorWithError:(TradableError * _Nonnull)error;
@end
...
@property (nonatomic, readonly) BOOL shouldOverlayData;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
@property (nonatomic, readonly, copy) NSDictionary<NSNumber *, NSNumber *> * _Nonnull values;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
SWIFT_PROTOCOL("_TtP11TradableAPI33TradableIndicatorSelectorDelegate_")
@protocol TradableIndicatorSelectorDelegate
/// Indicator Selector delegate hook for knowing what indicator was selected. Called when the widget has been dismissed.
- (void)tradableIndicatorSelectorDismissedWithRequestedIndicator:(TradableRequestedIndicator * _Nullable)requestedIndicator;
@end
...
- (double)getNextValidPriceFromPrice:(double)price SWIFT_WARN_UNUSED_RESULT;
...
- (double)getPreviousValidPriceFromPrice:(double)price SWIFT_WARN_UNUSED_RESULT;
...
- (double)getNextValidOrderSizeFromOrderSize:(double)orderSize SWIFT_WARN_UNUSED_RESULT;
...
- (double)getPreviousValidOrderSizeFromOrderSize:(double)orderSize SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
/// Specifies that ‘GET /instruments’ should be used to get a complete list of account instruments.
  TradableInstrumentRetrievalFullInstrumentList = 0,
/// Specifies that ‘POST /instrumentSearch’ should be used to search for instruments, and information about the instruments can be retrieved using ‘POST /instruments’
  TradableInstrumentRetrievalSearchTwoCharactersMinimum = 1,
};
...
@property (nonatomic, readonly, copy) NSString * _Nonnull matchedQuery;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
- (void)tradableInstrumentSelectorDismissedWithInstrumentSearchResult:(TradableInstrumentSearchResult * _Nullable)instrumentSearchResult;
@end
...
/// Spread bet.
  TradableInstrumentTypeSpreadBet = 5,
...
///
- (double)calculateRequiredMarginEstimateForQuantity:(double)quantity SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
///
- (void)getThumbnail:(void (^ _Null_unspecified)(UIImage * _Nullable))thumbCompleted;
/// Fetches a picture (160x160 pixels) and caches it.
/// \param picCompleted A closure containing the picture for this user.
///
- (void)getPicture:(void (^ _Null_unspecified)(UIImage * _Nullable))picCompleted;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
///
- (void)modifyWith:(TradableOrderModification * _Nonnull)orderModification completionHandler:(void (^ _Null_unspecified)(TradableError * _Nullable))completionHandler;
/// Cancels the order.
/// \param completionHandler A closure to be called when the response comes back, with an optional TradableError object.
///
...
///
- (void)getAffectedPositions:(void (^ _Null_unspecified)(NSArray<TradablePosition *> * _Nullable))completionHandler;
...
- (void)tradableOrderEntryDismissedWithOrder:(TradableOrder * _Nullable)order;
@end
...
///
/// \param priceModification The TradablePriceModification object containing the changes to be made to a protection.
///
/// \param completionHandler A closure to be called when the response comes back, with an optional TradableError object.
///
- (void)modifySingleProtectionOfType:(enum TradableSingleProtection)protectionType with:(TradablePriceModification * _Nonnull)priceModification completionHandler:(void (^ _Null_unspecified)(TradableError * _Nullable))completionHandler;
...
/// \param completionHandler A closure to be called when the response comes back, with an optional TradableError object.
///
- (void)modifyProtections:(TradableProtections * _Nonnull)protections completionHandler:(void (^ _Null_unspecified)(TradableError * _Nullable))completionHandler;
/// Cancels protections.
/// \param completionHandler A closure to be called when the response comes back, with an optional TradableError object.
///
...
typedef SWIFT_ENUM(NSInteger, TradablePriceType) {
/// Open.
  TradablePriceTypeOpen = 0,
/// High.
  TradablePriceTypeHigh = 1,
/// Low.
  TradablePriceTypeLow = 2,
/// Close.
  TradablePriceTypeClose = 3,
/// Median (high + low) / 2.
  TradablePriceTypeMedian = 4,
/// Typical (high + low + close) / 3.
  TradablePriceTypeTypical = 5,
/// Weighted close (high + low + 2 * close) / 2.
  TradablePriceTypeWeightedClose = 6,
};
...
- (NSArray<TradablePrice *> * _Nonnull)getPrices SWIFT_WARN_UNUSED_RESULT;
/// Returns an optional TradablePrice object for specified instrument ID.
- (TradablePrice * _Nullable)getPriceForInstrumentId:(NSString * _Nonnull)instrumentId SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end
...
SWIFT_CLASS("_TtC11TradableAPI19TradableProtections")
@interface TradableProtections : NSObject
/// Stop loss price.
...
///
- (void)tradableInstrumentSearchResultsReceivedWithSearchResults:(TradableInstrumentSearchResults * _Nonnull)searchResults;
@end
...
SWIFT_CLASS("_TtC11TradableAPI25TradableStartTrialRequest")
@interface TradableStartTrialRequest : NSObject
/// The id of the broker that the account is at.
@property (nonatomic, readonly) NSInteger brokerId;
/// The login for the account.
@property (nonatomic, readonly, copy) NSString * _Nonnull login;
...
SWIFT_CLASS("_TtC11TradableAPI27TradableSubscriptionRequest")
@interface TradableSubscriptionRequest : NSObject
/// The id of the broker that the account is at.
@property (nonatomic, readonly) NSInteger brokerId;
/// The login for the account.
@property (nonatomic, readonly, copy) NSString * _Nonnull login;
...
/// The Stripe token.
@property (nonatomic, readonly, copy) NSString * _Nonnull stripeToken;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
/// Creates an object with given parameters.
- (nonnull instancetype)initWithBrokerId:(NSInteger)brokerId login:(NSString * _Nonnull)login password:(NSString * _Nonnull)password email:(NSString * _Nonnull)email stripeToken:(NSString * _Nonnull)stripeToken OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end

enum TradableSubscriptionStatus : NSInteger;

/// A response received when making changes pertaining to a subscription.
SWIFT_CLASS("_TtC11TradableAPI28TradableSubscriptionResponse")
@interface TradableSubscriptionResponse : NSObject
/// The id of the broker that was subscribed to.
@property (nonatomic, readonly) NSInteger brokerId;
/// The login identifying the subscribed user.
@property (nonatomic, readonly, copy) NSString * _Nonnull brokerLogin;
/// The e-mail provided for receiving subscription-related information.
@property (nonatomic, readonly, copy) NSString * _Nonnull email;
/// The current status of the subscription.
@property (nonatomic, readonly) enum TradableSubscriptionStatus status;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
/// Creates an object with given parameters.
- (nonnull instancetype)initWithBrokerId:(NSInteger)brokerId brokerLogin:(NSString * _Nonnull)brokerLogin email:(NSString * _Nonnull)email status:(enum TradableSubscriptionStatus)status OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end

/// Possible states of subscription.
typedef SWIFT_ENUM(NSInteger, TradableSubscriptionStatus) {
/// The subscription is active.
  TradableSubscriptionStatusActive = 0,
/// The subscription is inactive.
  TradableSubscriptionStatusInactive = 1,
/// The subscription will be cancelled at the end of the period.
  TradableSubscriptionStatusCancelAtPeriodEnd = 2,
/// The subscription is cancelled.
  TradableSubscriptionStatusCancelled = 3,
};
...
@property (nonatomic, readonly) BOOL doesRequireUserInput;
...
@property (nonatomic, readonly, copy) NSString * _Nullable userInput;
/// Simple description of this object.
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
/// Creates an object with given parameters.
- (nonnull instancetype)initWithToken:(NSString * _Nonnull)token userInput:(NSString * _Nullable)userInput OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end

/// Possible frequencies of the updates.
typedef SWIFT_ENUM(NSInteger, TradableUpdatesFrequency) {
/// A new request is made as soon as the response comes back.
  TradableUpdatesFrequencyRealTime = 0,
/// A new request is made one second after the response comes back.
  TradableUpdatesFrequencyOneSecond = 1,
/// A new request is made two seconds after the response comes back.
  TradableUpdatesFrequencyTwoSeconds = 2,
/// A new request is made three seconds after the response comes back.
  TradableUpdatesFrequencyThreeSeconds = 3,
};


/// A request that should be sent to get prices or full account snapshot.
SWIFT_CLASS("_TtC11TradableAPI22TradableUpdatesRequest")
@interface TradableUpdatesRequest : NSObject
...
@property (nonatomic, readonly) BOOL includeOpenPositions;
...
@property (nonatomic, readonly, copy) NSString * _Nonnull description;
/// Creates an object with given parameters.
- (nonnull instancetype)initWithInstrumentIds:(NSArray<NSString *> * _Nonnull)instrumentIds includeOpenPositions:(BOOL)includeOpenPositions includeMarginFactors:(BOOL)includeMarginFactors OBJC_DESIGNATED_INITIALIZER;
/// A convenience initializer that creates TradableUpdateRequest object with a list of instruments.
- (nonnull instancetype)initWithInstruments:(NSArray<TradableInstrument *> * _Nonnull)instruments includeOpenPositions:(BOOL)includeOpenPositions includeMarginFactors:(BOOL)includeMarginFactors;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
@end

/// Possible types of updates.
typedef SWIFT_ENUM(NSInteger, TradableUpdatesType) {
/// Updates for account snapshot, orders, positions, prices and metrics. A list of instrument IDs for which the prices will be fetched should be specified for this updates type. If the list is empty, no prices will be retrieved.
  TradableUpdatesTypeFull = 0,
/// Updates just for account positions.
  TradableUpdatesTypePositions = 1,
/// Updates just for account orders.
  TradableUpdatesTypeOrders = 2,
/// Updates just for tick prices. A list of instrument IDs for which the prices will be fetched should be specified for this updates type. If the list is empty, no prices will be retrieved.
  TradableUpdatesTypePrices = 3,
/// Updates just for account metrics.
  TradableUpdatesTypeMetrics = 4,
};
...
///
/// \param side A preselected order side in the widget. Defaults to buy.
///
/// \param delegate An optional TradableOrderEntryDelegate that will be called when the widget is dismissed.
///
/// \param presentationStyle The widget’s presentation style. Defaults to UIModalPresentationStyle.overFullScreen.
///
...
/// \param uri OAuth flow redirect URL.
///
- (void)tradablePresentAuthenticationWebsiteForAppId:(uint64_t)appId withUri:(NSString * _Nonnull)uri showLogin:(BOOL)showLogin;
/// Presents a Two Factor Authentication Challenge dialog.
/// \param account The account for which to present the dialog with a challenge.
///
/// \param challenge The two factor authentication challenge.
///
- (void)tradablePresentTwoFactorAuthenticationChallengeDialogFor:(TradableAccount * _Nonnull)account with:(TradableTwoFactorAuthenticationChallenge * _Nonnull)challenge;
@end
